...
if not os.path.exists(f"{temp_path2}"):
    os.mkdir(f"{temp_path2}")
if not os.path.exists(f"{root_path}/{modelDir}"):
    os.mkdir(f"{root_path}/{modelDir}")
...
def checkUser():
    ...
    if bResult:
        aResponse = {"Status": 1, "user": bResult}
    else:
        aResponse = {"Status": 0}
